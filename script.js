...
// placeholder image
img.src =
	"https://assets.website-files.com/5f6bc60e665f54545a1e52a5/63d3f236a6f0dae14cdf0063_drag-image-here.png";
img.onload = function () {
	scaleFactor = 0.69;
...
function findClosestPoint(x, y) {
	...
	for (let i = 0; i < masterPoints.length; i++) {
		for (let j = 0; j < masterPoints[i].length; j++) {
			...
			if (dist < minDist && dist < 10 / scaleFactor) {
				// grab radius
				minDist = dist;
				closestPoint = [px, py];
				polygonIndex = i;
...
document
	.querySelector("#copyPythonButton")
...
canvas.addEventListener("mousemove", function (e) {
	...
	if (canvas.style.cursor == "crosshair") {
		...
		if (
			(points.length > 0 && drawMode == "polygon") ||
			(points.length > 0 && points.length < 2 && drawMode == "line")
		) {
			ctx.beginPath();
			ctx.lineJoin = "bevel";
			ctx.strokeStyle = rgb_color;
...
canvas.addEventListener("mousedown", function (e) {
	...
	if (editMode) {
		...
		if (point) {
			...
			//This part allows me to add new nodes and interact with them
			//In edit mode
			//ctrl + click over a node to add a new one in that line
			if (e.ctrlKey) {
				const polygon = masterPoints[selectedPolygonIndex];
				if (polygon.length >= 2) {
					const startPoint = polygon[selectedPointIndex];
					const endPoint = polygon[(selectedPointIndex + 1) % polygon.length];
					...
					// Insert the new node
					polygon.splice(selectedPointIndex + 1, 0, newNode);
			...
			//This part allows me to delete a specific node
			//In edit mode
			//alt + click over a node to delete it
			if (e.altKey) {
				const polygon = masterPoints[selectedPolygonIndex];
				polygon.splice(selectedPointIndex, 1);
...
document
	.querySelector("#normalize-checkbox")
...
function setEditMode(editEnabled) {
	...
	document
		...
		.forEach((el) => el.classList.remove("active"));
	document
		.querySelector(`#mode-${editEnabled ? "edit" : drawMode}`)
...
function toggleFullscreen() {
	...
	if (!isFullscreen) {
		...
		} else if (taskbarAndCanvas.webkitRequestFullscreen) {
			// Safari
			taskbarAndCanvas.webkitRequestFullscreen();
		} else if (taskbarAndCanvas.msRequestFullscreen) {
			// IE/Edge
